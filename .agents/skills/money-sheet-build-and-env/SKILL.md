...
| `_*_globals.ts` are `.ts`, not `.d.ts` | `clasp/tsconfig.json` has `skipLibCheck: true`, which silently skips ALL `.d.ts` (even hand-written), so a `.d.ts` mirror could drift undetected. Plain `.ts` files are still checked; mirrors derive types via `typeof import("./lib/…")` so drift = `tsc` error (issue #109, `clasp/AGENTS.md:29`). Never convert them to `.d.ts`. |
...
| AGENTS.md deploy branch is stale | Root AGENTS.md says deploy on push to `master`; all workflows actually trigger on `main`. |
