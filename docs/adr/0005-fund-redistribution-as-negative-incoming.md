...
- **Update (#136 follow-up, ADR-0012):** the mechanism for this is now the named `entryAmount` adapter (`frontend/src/lib/amountField.ts`), not a direction-conditional `allowNegative` boolean threaded through call sites — that shape was the direct cause of #134/#136. `splitEntry.ts`'s `validateLeg`/`validateSplit` call `entryAmount.validate`; `isSplitValid` no longer exists. See ADR-0012 for why.
