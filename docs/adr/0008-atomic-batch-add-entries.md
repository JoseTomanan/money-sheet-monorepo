...
- **Validate-then-write:** every leg is checked against the identical single-leg rules used by `addEntry` (tag/direction invariant, ISO date, finite amount) *before* the lock is acquired or any row is written. The first invalid leg rejects the whole batch with a validation error; nothing is written. There is no rollback path — validate-then-write makes a write failure mid-batch effectively unreachable short of a Sheets outage, so none was built.
- **IDs are assigned in array order**, leg 0 lowest, as one contiguous block reserved from the durable per-spreadsheet high-water mark (ADR-0016) — preserving the `^^` convention's "main leg owns the lowest ID in the run" without any client-side await/race choreography.
